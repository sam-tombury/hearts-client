<script>
	...
	const join = () => {
		...
		let listenUrl = new URL('/api/ws/games/' + gameID + '/seats/' + seatID + '/listen', window.location.href);
		listenUrl.protocol = listenUrl.protocol.replace('http', 'ws');
		const socket = new WebSocket(listenUrl.href)
		socket.onopen = () => {
			showEvent('Welcome, ' + name + '. The game will start once all players have joined.')
		}
	...
	const receiveHand = data => {
		...
		showEvent('Choose ' + data.passCount + ' cards to pass to ' + data.passTo + '. ' + data.firstLead + ' will lead first.')
	...
	const endTrick = data => {
		players = players.map(player => {
			if (player.id === data.winner.id) {
				player.currentHandScoringCards = [...player.currentHandScoringCards, ...data.scoringCards]
	...
	const endHand = data => {
		players = players.map(player => {
			player.score += data.find(result => result.player.id === player.id).pointsTaken
	...
	const sortHand = () => {
		hand.sort((card1, card2) => (card1.card.suit.order - card2.card.suit.order)*100 + (card1.card.value.order - card2.card.value.order))
		hand = hand
	}
	...
	const cardPlayed = data => {
		...
		showEvent('The ' + data.card.value.name + ' of ' + data.card.suit.name + ' was played')
...
<main>
	...
	{#if hand.length > 0}
		...
		<p>
			{#if passing}
				<button disabled={selectedCards.length !== passCount} on:click={pass}>Pass</button>
			{:else if playing}
				<button disabled={selectedCards.length !== 1} on:click={play}>Play</button>
			{/if}
		</p>
		{#each hand as {card, selected}}
			<Card on:click={() => toggleSelected(card)} {...card} selected={selected}/>
		{/each}
	{/if}
</main>
